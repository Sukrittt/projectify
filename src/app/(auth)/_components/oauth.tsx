...
type OAuthProvider = {
  name: string;
  icon: keyof typeof OAuthIcon;
  strategy: OAuthStrategy;
};

const oauthProviders = [
  { name: "Google", strategy: "oauth_google" as const, icon: "oauth_google" },
  { name: "GitHub", strategy: "oauth_github" as const, icon: "oauth_github" },
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {oauthProviders.map((provider, index) => {
        const Icon = OAuthIcon[provider.icon];
        ...
        return (
          ...
          >
            {isLoading === provider.strategy ? (
              <LoaderDot className="pb-1.5 pr-1" dotClassName="bg-black" />
            ) : (
...
} as Record<OAuthStrategy, (props: LucideProps) => JSX.Element>;
